...
el("nextsong").onclick = function (e) {
  if (loaded) {
    currentSong++;
    currentSong =
      currentSong > player.totalSongs ? player.totalSongs : currentSong;
    player.playSong(currentSong);
    drawVisual();
  }
};
...
function drawVisual() {
  ...
  let scale = player.apu.p1ConstantVolume
    ? player.apu.p1Volume
    : player.apu.p1Decay;
  ...
  ctx.fillRect(xPos + 16, 96, 16, 120);
  ctx.fillStyle = yellow;
  scale = player.apu.p2ConstantVolume
    ? player.apu.p2Volume
    : player.apu.p2Decay;
  ...
  ctx.fillStyle = yellow;
  scale =
    player.apu.triCounter === 0 || player.apu.triLinearCounter === 0 ? 0 : 1;
  scale = scale * 120;
  ctx.fillRect(xPos, 216 - scale, 16, scale);
  ctx.fillStyle = blue;
  ...
  ctx.fillRect(xPos + 16, 96, 16, 120);
  ctx.fillStyle = yellow;
  scale = player.apu.noiseConstantVolume
    ? player.apu.noiseVolume
    : player.apu.noiseDecay;
